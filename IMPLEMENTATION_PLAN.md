...
**M02-02: Content-addressed server store.** The local filesystem store now stages each blob in an exclusive same-directory temporary file and publishes it with an atomic no-clobber hard link; `tests/test_store_cli.sh` races twelve identical puts and verifies one complete object with no staging residue. Digest verification blocks publication from missing or corrupted blobs. The `evidence_references` PostgreSQL command performs a fixed read-only scan capped at 100,001 evidence rows and reports valid content-addressed keys, malformed-key count, and truncation; `tests/test_postgres_cli.sh` covers its result and query-failure path. Shared-blob distribution and reference-aware orphan deletion remain open; cleanup must fail closed on malformed or truncated snapshots and still needs protection against references added between discovery and deletion.
