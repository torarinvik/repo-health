...
**M02-01: PostgreSQL persistence.** Create source, collection, evidence, event, identity-placeholder, metric, and job tables. Add migration tests and transactional repository methods. Enforce source-scoped uniqueness. The target contract now lives in `db/migrations/001_initial.sql`: it covers the architecture table groups, visibility-scoped evidence, source-native uniqueness, bitemporal checks, adjacency/evidence/metric indexes, fenced lease claim/heartbeat/finish functions, and cursor-advancing functions that refuse partial pages. The `rh_commit_collection_page_events` function validates bounded event and typed-subject arrays, registers or verifies each subject's immutable metadata, absorbs source-native event duplicates, and commits subjects, event rows, page metadata, and cursor advancement atomically; a malformed row or subject conflict rolls back the transaction. The idempotent `rh_register_evidence_object` function accepts bounded SHA-256 metadata and rejects identity or immutable-metadata conflicts. The `register_evidence` CLI operation reads a content-addressed blob from `RH_EVIDENCE_ROOT`, verifies its FNV-1a name, computes SHA-256, and binds only the resulting metadata to PostgreSQL; the blob itself remains in the configured evidence store. `tests/test_migrations.sh` validates the migration vocabulary and safety invariants, while `tests/test_migrations_live.sh` applies the migration to an ephemeral PostgreSQL 16 container and exercises evidence replay/conflict, subject creation/conflict, stale fencing, terminal finish, complete-only cursor advancement, duplicate-event replay, duplicate-page rejection, and malformed-page rollback when `RH_PG_MIGRATION=1`. `src/rh_postgres.elisa` provides parameter-bound, timeout-bounded methods for evidence registration, page/event-page commit, and PostgreSQL job claim/heartbeat/finish, distinguishing new registrations/commits, duplicates, claimed/empty queues, and current/stale tokens. `rh_cli postgres` exposes those operations through `src/rh_postgres_report.elisa` and versioned `rh-postgres-command/1` inputs; it reads connection settings only from `RH_DATABASE_URL` and emits `rh-postgres-result/1`. `tests/test_pg_adapter.sh` exercises the dynamic `libpq` ABI with a fake library, `tests/test_postgres_cli.sh` covers the command path and failure gates, and `tests/test_pg_adapter_live.sh` verifies the adapter against PostgreSQL when `RH_PG_ADAPTER=1` and `libpq` is installed. Source-instance/run setup and actor-account registration remain prerequisites; shared-blob distribution and wiring PostgreSQL into active ingestion remain open, and the filesystem store remains the active runtime.
