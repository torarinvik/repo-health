...
**M02-01: PostgreSQL persistence.** Create source, collection, evidence, event, identity-placeholder, metric, and job tables. Add migration tests and transactional repository methods. Enforce source-scoped uniqueness. The target contract now lives in `db/migrations/001_initial.sql`: it covers the architecture table groups, visibility-scoped evidence, source-native uniqueness, bitemporal checks, adjacency/evidence/metric indexes, fenced lease claim/heartbeat/finish functions, and a cursor-advancing page function that refuses partial pages. `tests/test_migrations.sh` validates the migration vocabulary and safety invariants, while `tests/test_migrations_live.sh` applies the migration to an ephemeral PostgreSQL 16 container and exercises stale fencing, terminal finish, complete-only cursor advancement, and duplicate-page idempotence when `RH_PG_MIGRATION=1`. `src/rh_postgres.elisa` now provides parameter-bound, timeout-bounded methods for complete-page commit and the PostgreSQL job claim/heartbeat/finish functions, distinguishing new commits, duplicates, claimed/empty queues, and current/stale tokens. `tests/test_pg_adapter.sh` exercises these dynamic `libpq` ABI paths with a fake library; `tests/test_pg_adapter_live.sh` verifies the operations against PostgreSQL when `RH_PG_ADAPTER=1` and `libpq` is installed. Event/evidence persistence, runtime configuration, and wiring into ingestion remain open; the filesystem store remains the active runtime.
