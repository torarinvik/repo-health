...
The new `rh_cli pylock --input <pylock.toml> --out <file>` path adds a bounded PEP 751 audit projection under `rh-pylock-audit/1`, bound to the exact input bytes by SHA-256. It preserves package names, versions, package markers, `requires-python`, and package-to-package references expressed by name plus optional version. References resolve only when those fields identify one package entry; missing, ambiguous, and source-specific references remain distinct. This follows the research paper's cautions about dependency context and directness: PEP 751 says its dependency links are informational and does not encode project-root dependencies, so the result is deliberately rootless and is not accepted as `rh-dep-graph/1` or OSV input. Marker evaluation, artifact hashes, source locators, environment/group selection, and complete TOML validation remain open; the report counts unprojected fields and tables. Full integration with project manifests and artifact observations remains a follow-up, and complete lockfile coverage remains open. (PEP 751; research paper §16.)
