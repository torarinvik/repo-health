...
**M03-02: Artifact identities.** Store expected and observed digests independently from version labels. Model multiple artifacts per package version and changed bytes under the same label. Lockfile checksums and integrity strings are now emitted as `expected_digest`; `observed_digest` is explicitly null until archive bytes are verified. Multiple artifact records per package version remain open.
