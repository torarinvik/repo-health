...
**M03-02: Artifact identities.** Store expected and observed digests independently from version labels. Model multiple artifacts per package version and changed bytes under the same label. The graph now stores artifact records separately from resolution nodes; multiple records may reference one package node, and each record carries expected/observed digests plus `match`/`changed`/`unknown` state. Cargo/npm/Go parsers populate expected digests from lockfile evidence. `rh_cli artifact-observe` hashes user-supplied local Cargo archive bytes with SHA-256 or npm tarball bytes with SHA-256/SHA-512 SRI Base64, then emits `rh-artifact-observation-result/1` bound to the graph digest and artifact ID; it never extracts or executes the archive. npm integrity metadata may contain multiple SRI tokens: this verifier selects SHA-512 when present, otherwise SHA-256 only when no SHA-384 token is present, and matches if any selected token validates while retaining the original expected string. The observation is retained as a separate sidecar, so the source graph remains immutable. Other ecosystems, Go module hash semantics, SHA-384-only npm integrity, integrity strings without SHA-256/SHA-512, and SRI options remain unsupported by this verifier. The dependency digest-coverage metric counts expected values and does not claim byte verification.
