...
**M02-01: PostgreSQL persistence.** Create source, collection, evidence, event, identity-placeholder, metric, and job tables. Add migration tests and transactional repository methods. Enforce source-scoped uniqueness. The target contract now lives in `db/migrations/001_initial.sql`: it covers the architecture table groups, visibility-scoped evidence, source-native uniqueness, bitemporal checks, adjacency/evidence/metric indexes, fenced lease claim/heartbeat/finish functions, and cursor-advancing functions that refuse partial pages and require a source-matched running job whose input manifest names the same collection run, with the current unexpired fencing token; the page and job rows stay locked through commit. The `rh_begin_collection_run` function atomically creates or verifies a source configuration and its initial collection run; exact retries are idempotent and changed metadata rolls back. The `rh_commit_collection_page_events` function validates bounded event, typed-subject, and source-scoped actor arrays, registers or verifies their immutable metadata, absorbs source-native event duplicates, and commits entities, accounts, event rows, page metadata, and cursor advancement atomically; a malformed row or metadata conflict rolls back the transaction. The idempotent `rh_register_evidence_object` function accepts bounded SHA-256 metadata and rejects identity or immutable-metadata conflicts. The `begin_collection_run` CLI operation binds source and run metadata through `rh_cli postgres`; `register_evidence` verifies a content-addressed blob from `RH_EVIDENCE_ROOT`, computes SHA-256, and binds only the resulting metadata to PostgreSQL, while the blob itself remains in the configured evidence store. `tests/test_migrations.sh` validates the migration vocabulary and safety invariants, while `tests/test_migrations_live.sh` applies the migration to an ephemeral PostgreSQL 16 container and exercises source/run replay and conflicts, evidence replay/conflict, subject/actor creation and conflict, run-mismatched, stale-token, and expired-token page rejection, terminal finish, complete-only cursor advancement, duplicate-event replay, duplicate-page rejection, and malformed-page rollback when `RH_PG_MIGRATION=1`. `src/rh_postgres.elisa` provides parameter-bound, timeout-bounded methods for source/run initialization, evidence registration, page/event-page commit, and PostgreSQL job claim/heartbeat/finish, distinguishing new registrations/commits, duplicates, claimed/empty queues, and current/stale tokens. `rh_cli postgres` exposes those operations through `src/rh_postgres_report.elisa` and versioned `rh-postgres-command/1` inputs; it reads connection settings only from `RH_DATABASE_URL` and emits `rh-postgres-result/1`. `tests/test_pg_adapter.sh` exercises the dynamic `libpq` ABI with a fake library, `tests/test_postgres_cli.sh` covers the command path and failure gates, and `tests/test_pg_adapter_live.sh` verifies source/run setup and the adapter against PostgreSQL when `RH_PG_ADAPTER=1` and `libpq` is installed. Shared-blob distribution and wiring PostgreSQL into active ingestion remain open, and the filesystem store remains the active runtime.

**M02-02: Content-addressed server store.** The local filesystem store now stages each blob in an exclusive same-directory temporary file and publishes it with an atomic no-clobber hard link; `tests/test_store_cli.sh` races twelve identical puts and verifies one complete object with no staging residue. Digest verification blocks publication from missing or corrupted blobs. Database-backed reference discovery, server/shared-blob distribution, and reference-aware orphan cleanup remain open.
