...
packet = {
    ...
    "limitations": [
        "HTTP listener serves a report directory read-only on loopback (M06-01), exposes a bounded POST /api/query path for rh-query-input/1 pagination, and serves a server-rendered accessible report page at /_report.html (M06-03); a full store-backed dynamic site remains outside this slice",
        ...
        "forge normalizers span GitHub, GitLab, Gitea, Forgejo and Bitbucket Cloud (captured payloads plus guarded --url transport with retained source/status/error evidence; no provider-specific auth or pagination); review workflows span mbox/patch series and Gerrit changes (fixtures, no live Gerrit); native VCS: Mercurial, Subversion and Fossil have bounded --repo collectors with retained source/stderr evidence (the local gate uses controlled shims because the tools are absent); non-PR workflow is subject-level patch-series grouping with trailer roles; release-only sources report releases/artifacts with guarded --url capture and no history; Python/PEP 440 version semantics has a prototype requirements.txt parser; dependency parsers span Cargo, npm, PyPI, Go, RubyGems and Composer (declared-requirement subsets); registry enrichment includes a bounded PyPI project JSON adapter plus generic guarded URL capture; CycloneDX and SPDX 2.3 inventory inputs support guarded --url capture; distribution data, archives and further ecosystems/inventory formats are not yet implemented (M08)",
