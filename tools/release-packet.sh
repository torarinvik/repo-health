...
packet = {
    ...
    "limitations": [
        "HTTP listener serves a report directory read-only on loopback (M06-01), frames split requests through a bounded header and Content-Length read, exposes a bounded POST /api/query path for rh-query-input/1 pagination and capped upstream/downstream graph traversal, fixed report-backed JSON resources for project/metrics/continuity/graph/findings/evidence/scan-status, verified GET/HEAD content-addressed blobs below evidence/<16-hex-digest>, and a server-rendered accessible report page at /_report.html (M06-03); durable background graph workers remain outside this slice",
