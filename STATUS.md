...
mismatched revisions rejected). Yarn Classic v1 is a fallback when no npm
lockfile exists; it maps exact name/range selectors, retains dependency scopes
and fingerprinted resolved locators, and rejects Berry revisions, aliases,
malformed entries and unknown fields. Explicit
...
`package-lock.json` or `npm-shrinkwrap.json` (npm precedence), falling back to
Yarn Classic v1 `yarn.lock` only when neither npm lock exists, `requirements.txt`, bounded PEP 621 `pyproject.toml`,
