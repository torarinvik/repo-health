...
| M09 catalog expansion | `in_progress` | admission lint (`tools/metric-lint.sh`) over 212 definitions (210 `implemented`, 2 `prototype`, 0 `planned`); experimental prototypes remain outside the runtime registry (see below) |
...
available) while published definitions are present. Catalog state: 212
definitions = 210 `implemented`, 2 `prototype`, 0 `planned`; the 360 target
...
"implemented"`** (computed on the real scan path). It lists 210 reachable
...
## Metric catalog status (210 implemented definitions; 226 Architecture keys lack an implemented definition)
...
| 10 | Extend to the initial 30–40 metric subset | `implemented` (210 metric definitions are implemented; registry/status agreement and admission lint pass in `tests/test_m00.sh`) |
