...
true stage. **225 of the 360 Architecture metric keys lack an implemented definition;
135 Architecture keys have at least one implemented definition. The runtime
catalog below contains 224 implemented definitions across 223 distinct keys.**

## Milestones

| Milestone | Stage | Evidence |
|---|---|---|
| M00 contracts/oracles | `in_progress` | `src/rh_*.elisa`, `build/test_oracles` (`ORACLES OK`), `tests/test_m00.sh`, 226 explicitly classified metric definitions, CHAOSS Contributor Absence Factor crosswalk and negative admission controls (`tests/test_metric_admission.sh`); staged forge normalization now emits a provider/capability field map with preserved/transformed/inferred/discarded/unsupported states, exact configuration digest, and final-event page/ordinal provenance and source page evidence IDs with last-observation replacement (`tests/test_staged_forge_cli.sh`); all five forge repository normalizers, ecosyste.ms package lookup, deps.dev version enrichment, registry metadata normalization, CycloneDX/SPDX inventory normalization, the PEP 751 lock audit and local artifact-observation projections, dependency-resolution instance normalization, single-query and graph-batch OSV normalization, and Scorecard findings normalization now emit digest-bound field transformation reports with configuration/input/output SHA-256 and field state mappings; equivalent reports for other non-forge adapters remain open |
| M01 safe Git scan | `implemented` | `build/rh_cli` (scan/replay/registry), `tests/test_m01.sh`; one tracked gap (see below) |
| M02 ingestion/hosts | `in_progress` | `RhJson`/`RhForge`/`RhForgeEvents`/`rh_store`/`rh_connector`/`rh_job`, `test_forge`/`test_store`/`test_job`, `tests/test_m02.sh`, `rh_cli store put|verify`, `rh_cli store lease` (`tests/test_lease_cli.sh`), `rh_cli ops backup|verify|restore` (`tests/test_store_cli.sh`), `rh_cli forge normalize` (`tests/test_forge_cli.sh`; optional `RH_GITHUB_TOKEN` is sent only to `api.github.com`), bounded captured issues/proposals/reviews/releases plus paginated live GitHub issues, pull requests, and releases through `rh_cli forge events` (`rh-forge-events-result/1`, `tests/test_forge_events_cli.sh`; live collection is capped at 10 pages per capability and retains raw page/URL/status/error evidence; `--review-pull N` fetches bounded pull-request-scoped reviews and skips unrelated capabilities as `not_attempted`; `--review-page N` resumes the returned numeric review cursor for that explicit repository and pull request; repository-wide reviews use bounded 1–10 PR batches, 1–10 review pages per PR, a shared 3 MiB response cap, and validated per-PR/pull-list cursors through `--review-repository` and `--resume-from`; each result is one segment and GitHub pull-list pagination is not an atomic snapshot; release drafts are filtered), `rh_cli connector check` (`tests/test_connector_cli.sh`, `rh-connector-instance/1`) plus `rh_cli connector probe --github-repo owner/repo` (`rh-github-capability-probe-result/1`; probe result omits counts, 403/404 remain inconclusive), `connector probe --review-pull N` (`rh-github-review-capability-probe-result/1`), and authenticated `connector probe --collaborators` (`rh-github-collaborator-probe-result/1`; one row only, names retained only in local evidence), and `connector probe --gitlab-project group/path --all` (`rh-gitlab-capability-matrix/1`; three fixed routes with host-scoped optional token auth and per-route coverage mapping) plus `rh_cli connector traffic-observe` (`rh-github-traffic-observation/1`; exact-response SHA-256, reported daily buckets, and explicit non-additive 14-day snapshot), `rh_cli job classify|run` (`tests/test_job_cli.sh`, `rh-job-next/1`+`rh-sched-result/1`) with a bounded scheduler (enqueue/claim/succeed/fail/cancel/reclaim), host quota and backoff; `rh_cli reconcile` (`tests/test_reconcile_cli.sh`, `rh-reconcile-result/1`) full-reconcile scheduling (due/horizon/debt/overlap), `rh_cli snapshot-reconcile` (`src/rh_snapshot_reconcile.elisa`, `tests/test_snapshot_reconcile_cli.sh`, `rh-snapshot-reconcile-result/1`) scope-exact absent/present/unconfirmed projection with complete/empty-only clearing, `rh_cli worker tick` (`tests/test_worker_cli.sh`, `rh-worker-plan/2`) one bounded deterministic scheduling pass with due full reconciles before incremental work and source rotation by time, `tools/worker-daemon.sh` digest-gated process wrapper (`tests/test_worker_daemon.sh`), and `rh_cli ingest` (`tests/test_ingest_conformance_cli.sh`, `rh-ingest-result/1`) durable event-before-cursor replay with fencing and explicit empty/failure/partial states; `src/rh_postgres.elisa` plus `rh_cli postgres` expose parameter-bound source/run initialization, evidence registration, page/event-page commit, generic and targeted collection-job claims, enqueue, heartbeat, and finish through versioned commands and `RH_DATABASE_URL` (`tests/test_pg_adapter.sh`, `tests/test_postgres_cli.sh`, opt-in `tests/test_pg_adapter_live.sh`); source/run retries verify immutable metadata, event pages atomically register typed subjects and source-scoped actors, and durable pages require a source-matched, run-bound live job token; `rh_cli ingest --postgres [--root <store-dir>]` (`rh-postgres-ingest-input/1` -> `rh-postgres-ingest-result/1`) preflights normalized pages before connecting, then performs run setup, enqueue, targeted claim, atomic event-page/cursor commits, and fenced finalization for complete, partial, failed, and canceled collection outcomes; incomplete runs may commit only supplied complete/empty pages or no pages and retain coverage/error metadata; optional `evidence_objects` bind verified `<root>/evidence` blobs to PostgreSQL before run setup under the evidence-root guard, with fake-libpq coverage and an opt-in live test; staged forge normalization carries final-event-to-page/ordinal and evidence-ID provenance through duplicate replacement; `rh_cli postgres` now accepts `commit_staged_normalization` to persist a completed normalization with an immutable replay manifest, scope-qualified source object identity, exact normalized-event output digest, and database checks for complete runs, retained staged rows, page ordinals, matching evidence, and replay payload equality against committed canonical events; `tests/test_postgres_cli.sh` and `tests/test_pg_adapter.sh` cover preflight, parameter binding, commit, and replay, while `RH_PG_MIGRATION=1 tests/test_migrations_live.sh` exercises the real transaction, exact replay, output conflict, page-evidence mismatch, and partial-run rejection; Bitbucket issue/pull-request probes and approved Gitea/Forgejo instance probes are implemented; controlled-instance rehearsal of complete GitHub/GitLab collaborator-permission inventories and provider normalization beyond the forge adapter remain open; the filesystem replay-to-PostgreSQL live bridge is implemented in `tests/test_pg_adapter_live.sh` but its Docker-backed execution remains unverified |
| M03 packages/advisories | `in_progress` | `src/rh_package.elisa` (Cargo + npm, pnpm v9.0, Yarn Classic v1 + PyPI requirements/PEP 621 + Go modules + RubyGems + Composer + NuGet `packages.config` and `packages.lock.json` + Maven `pom.xml`) + `src/rh_artifact_observation.elisa` + `src/rh_go_zip.elisa` + `src/rh_inventory.elisa` + `src/rh_inventory_report.elisa` + `src/rh_registry_meta.elisa` + `src/rh_registry_pypi.elisa` + `src/rh_registry_npm.elisa` + `src/rh_registry_crates.elisa` + `src/rh_registry_rubygems.elisa` + `src/rh_registry_nuget.elisa` + `src/rh_registry_meta_report.elisa` + `src/rh_depsdev.elisa` + `src/rh_ecosystem_lookup.elisa` + `src/rh_resolution.elisa` + `src/rh_parser_diff.elisa` + `src/rh_adapter_manifest.elisa` + `src/rh_osv_query.elisa` + `src/rh_pylock.elisa` (PEP 751 rootless audit projection with environment/group arrays, UTC artifact upload times, and archive subdirectories, `rh_cli pylock`, `rh-pylock-audit/1`) + `src/rh_pylock_observation.elisa` (`rh_cli pylock-observe`, `rh-pylock-artifact-observation-result/1`) + `src/rh_pylock_observation_transformation.elisa` (digest-bound transformation sidecar) + `src/rh_pylock_context.elisa`/`src/rh_pylock_context_transformation.elisa` (`rh_cli pylock-evaluate`, `rh-pylock-marker-evaluation/1`, context and PEP 440 compatibility states, validated extras/public/default groups, digest-bound sidecar) + `test_package` (`PACKAGE OK`) + `test_osv_query`, `rh_cli deps` publishes `dependencies.unsupported_range_count`, `rh_cli artifact-observe` publishes graph-bound Cargo SHA-256 archive, npm SHA-256/SHA-384/SHA-512 SRI tarball, Go `.mod` and module ZIP h1 evidence (`rh-artifact-observation-result/1`, `rh-go-mod-observation-result/1`, and `rh-go-zip-observation-result/1`), `rh_cli inventory` publishes `rh-inventory/1`, `rh_cli registry-meta` publishes `rh-registry-meta-result/1` for canonical, PyPI, npm, crates.io, RubyGems, and bounded NuGet registration metadata, `rh_cli depsdev` publishes `rh-depsdev-enrichment/1`, `rh_cli ecosystem lookup` publishes `rh-ecosystem-lookup-result/1` from bounded captures or one explicitly requested fixed-route live lookup, with exact-coordinate candidate accounting and source/status plus CC BY-SA 4.0 attribution, licence, and modification provenance, `rh_cli resolution` publishes `rh-resolution-instance/1`, `rh_cli parser-diff` publishes `rh-parser-diff/1`, and `rh_cli adapter-manifest` publishes `rh-adapter-manifest-result/1`; `tests/test_m03.sh`, `tests/test_deps_cli.sh` (Cargo, npm, pnpm v9.0, and Yarn lockfile coverage), `tests/test_artifact_observation_cli.sh` (digest-bound transformation sidecar), `tests/test_inventory_cli.sh`, `tests/test_registry_meta_cli.sh`, `tests/test_depsdev_cli.sh`, `tests/test_ecosystem_lookup_cli.sh`, `tests/test_resolution_cli.sh`, `tests/test_parser_diff_cli.sh`, `tests/test_adapter_manifest_cli.sh`, `tests/test_osv_query_cli.sh`, `tests/test_pylock_cli.sh` (PEP 751 context arrays, artifact sizes/upload times and archive subdirectories, SHA-256/384/512 local-byte checks, transformation sidecar digests and path privacy); OSV supports bounded package/version and full commit queries plus graph batches of up to 64 nodes; graph result `/2` follows only pending per-query cursors for at most four rounds and retains indexed request/response evidence; opt-in `--hydrate-advisories` result `/3` fetches up to 64 distinct full records and 32 MiB of bodies with a 120-second total and 10-second per-request cap, validates returned IDs, retains per-record evidence, and reports any gap as partial. Complete lockfile coverage and further provider adapters remain open |
| M04 continuity/identity | `in_progress` | `src/rh_identity.elisa` + `src/rh_identity_report.elisa` + `src/rh_roles.elisa` + `src/rh_roles_report.elisa` + `src/rh_roles_provider.elisa` + `src/rh_continuity.elisa` + `src/rh_continuity_scan.elisa` + `src/rh_role_grain.elisa`, `test_continuity` (`CONTINUITY OK`)/`test_continuity_scan`, `rh_cli identity`/`roles`/`roles-import`/`continuity` publish `rh-identity-result/1`/`rh-roles-result/1`/`rh-provider-roles-input/1`/`persistence.retained_90d`+`concentration.change_*`, and `rh_cli role-grain` publishes `rh-role-grain-result/1` with one-event/one-role cardinality plus as-known/current-corrected replay; `tests/test_m04.sh`, `tests/test_continuity_cli.sh`, `tests/test_identity_cli.sh`, `tests/test_roles_cli.sh`, `tests/test_roles_provider_cli.sh`, `tests/test_role_grain_cli.sh`, `tests/test_succession_cli.sh`; `rh_cli succession` supports versioned declared handover follow-up windows with right-censoring and `rh-succession-input/3` activity-derived primary actor changes with tie/empty-month unknowns and no actor-index publication; identity reports stratify actor kinds and sources (provider, project, operator, unknown), retain validated per-actor observation times and evidence references only in restricted output, publish public known/unknown time and reference-presence coverage, and reject duplicate source/instance/native-ID tuples; optional `identity --evidence-store <root/evidence>` verifies actor-kind references and requires accepted/rejected/revoked links to carry reviewer, nonnegative review time, and verified evidence, and cross-source links to carry a review rationale; its digest-bound `rh-identity-evidence-verification/1` sidecar reports reviewed-link counts and writes restricted, input/output-bound `rh-identity-review-notices/1` artifacts for cross-source decisions; without a store, both sidecars explicitly report unverified and empty counts; `tests/test_identity_cli.sh` covers verified/corrupt blobs and reviewed-link fail-closed behavior; project identity publication resolves ordered half-open actor-kind histories at an explicit as-of time, aggregates overlapping kind conflicts as unresolved and source conflicts as unknown, and never publishes actor indices or assertion rows; role reports break release, merge, and review event totals out by actor type and leave missing classifications unknown; roles can consume the identity report revision and `cluster_id_by_actor` snapshot, resolving action identity-ledger indexes into corrected clusters for distinct release/review actor and concentration metrics while preserving source-native attribution; explicit per-action actor-key overrides remain revision-bound; continuity applies the repository `.mailmap` to project-local activity while retaining raw author fields; bounded captured GitHub/GitLab permission imports are implemented with duplicate replacement and opaque pagination retention; token-gated GitHub collaborator and GitLab project-member acquisition carry provider-specific scope and short-page completeness into roles reports, with bounded pagination and validated `--resume-from` cursors that merge actor IDs while preserving prior observation times; static and server-rendered continuity UI is covered by M06 tests |
| M05 temporal/downstream | `in_progress` | `src/rh_graph.elisa` + `src/rh_downstream.elisa` + `src/rh_downstream_report.elisa` + `src/rh_projection_store.elisa` + `src/rh_mapping.elisa` + `src/rh_coverage.elisa` + `src/rh_adoption.elisa` + `src/rh_population.elisa` + correction-aware `current_event_ledger` in `src/rh_role_grain.elisa` (as-known ledger retained; replaced actors applied; retractions omitted) + `src/rh_maintenance_exposure.elisa` (`rh-maintenance-exposure-input/1` → `rh-maintenance-exposure-result/1`, verified role-result blob references and accepted role-evidence overlap; actors withheld), `test_m05` (`M05 OK`), `rh_cli downstream` publishes `rh-downstream/1` (mirror dedup R012 + per-metric intrinsic join R011) and optionally persists `rh-projection-snapshot/2` IDs; `--identity-revision` is carried into reports and snapshot manifests so revised identity projections receive distinct content IDs; accepted mirror/migration mappings respect `reviewed_at` cutoffs and remap edge endpoints before traversal; private group members hide the full canonical group (`tests/test_downstream_cli.sh`); `rh_cli mapping` publishes `rh-mapping-result/1` and `--mapping` carries reviewed revision into the projection; `rh_cli population` publishes `rh-population-result/1` with bounded selected dependents, family deduplication, path witnesses, per-metric coverage/distributions, unresolved mappings, and policy unknowns; `tests/test_m05.sh` and `tests/test_downstream_cli.sh` cover exact node-budget enforcement and report truncation only when visible dependents are actually hidden; `tests/test_mapping_cli.sh`, `tests/test_coverage_cli.sh`, `tests/test_adoption_cli.sh`, `tests/test_population_cli.sh`, `tests/test_role_grain_cli.sh` (corrected event ledger), `tests/test_maintenance_exposure_cli.sh` (reviewed evidence join, overlap, unknowns, privacy); reviewer authorization remains an external governance prerequisite; source-specific validity intervals now publish `rh-coverage-result/1` with explicit null endpoints and source capability states, requiring nonnegative collection times and rejecting capability known-times later than collection while preserving epoch zero (`tests/test_coverage_cli.sh`), while `rh-adoption-result/1` publishes separate first/last observation and confirmed introduction/removal evidence, nullable observed-upgrade/supported-line states, and confirmed-removal/right-censored/unknown duration components with epoch zero distinct from null; seven aggregate counts, the exact supported-version adoption share, separate five-bucket confirmed/right-censored duration distributions, observed/right-censored release-to-first-qualifying-snapshot lag distributions, and caller-supplied coverage intervals, reconstructed snapshot-run histories, and `rh_cli adoption --store-root` ingestion from persisted filesystem coverage intervals report gap counts and uncovered seconds; missing store history remains unknown and malformed interval logs fail closed; the bounded `rh_cli adoption --postgres` path now reads parameter-bound collection-run windows from `RH_DATABASE_URL`, marks only succeeded complete/empty runs complete, and fails closed on over-limit/subsecond histories; adoption records can join up to 16 ordered source histories for one capability, with incomplete spans and cross-source order enforced fail-closed; combined histories are bounded to 4,096 intervals and 1 MiB (`tests/test_adoption_cli.sh`, `tests/test_pg_adapter.sh`) |
| M06 API/policy/corrections | `in_progress` | `src/rh_policy.elisa` + `src/rh_policy_parse.elisa` + `src/rh_correction.elisa` + `src/rh_correction_report.elisa` + `src/rh_correction_access.elisa` (`rh-correction-evidence-policy/1`, digest-bound `rh-correction-evidence-verification/1`) + `src/rh_notify.elisa` + `src/rh_notify_report.elisa` + `src/rh_query.elisa` + `src/rh_query_report.elisa` + `src/rh_render.elisa` + `src/rh_findings.elisa` + `src/rh_findings_transformation.elisa` + `src/rh_lineage.elisa` + `src/rh_drilldown.elisa`, `test_m06`/`test_m06b`, `rh_cli policy`/`correct`/`notify`/`query`/`render`/`findings`/`lineage`/`drilldown` publish `rh-policy-result/1`/`rh-policy-state/1`/`rh-corrections-result/1`/`rh-corrections-state/1`/`rh-notify-result/4`+`rh-notify-state/2`/`rh-query-result/1`/`repo-health-report-html/1`/`rh-findings-result/1`/`rh-lineage-result/1`/`rh-evidence-drilldown/1`, `rh_cli serve /_report.html` is server-rendered; `tests/test_m06.sh`, `tests/test_policy_cli.sh`, `tests/test_correction_cli.sh`, `tests/test_notify_cli.sh`, `tests/test_query_cli.sh`, `tests/test_render_cli.sh`, `tests/test_findings_cli.sh` (typed outcomes and transformation-sidecar digests), `tests/test_lineage_cli.sh`, `tests/test_drilldown_cli.sh`; dynamic UI tables, fixed report-backed API resource routes, capped upstream/downstream query traversal with truncation metadata, verified `/api/store/<16-hex-digest>` evidence lookup, structured findings, lineage-preserving evidence drill-down, and durable state-store wiring are implemented; corrections may opt into an operator-managed evidence policy that requires one active purpose-bound grant per digest and allowlisted reviewer IDs, while trusted policy delivery and reviewer identity authentication remain deployment responsibilities; successful checks emit exact input/policy digests and evaluation time; expiry denies use but does not delete retained blobs; accepted corrections emit idempotent `rh-correction-notices/1` artifacts; external message transport remains open |
| M07 beta gate | `in_progress` | `src/rh_ops.elisa` + `src/rh_ops_report.elisa` (`rh_cli ops monitor|quota`) + `src/rh_privacy.elisa` + `src/rh_privacy_report.elisa` (`rh_cli privacy`) + `src/rh_sha256.elisa` (SHA-256/HMAC, M07-04 sign/verify) + transport guard (`src/rh_git.elisa`) + `ops/source-review-register.json`, `test_m07` (`M07 OK`), `test_m07_sha`, `tests/test_m07.sh`/`test_m07_sanitizer.sh`/`test_m07_fuzz.sh`/`test_m07_sha.sh`/`test_ops_cli.sh`/`test_privacy_cli.sh`; outbound guarded API requests identify repo-health/0.2.0 and the public issue/contact route in User-Agent; current fetch paths are fixed-route lookups, not recursive crawling, so exclusion-list support remains a prerequisite for crawler expansion; the Elisa parser target has a bounded 1,000-input LLVM libFuzzer gate plus a completed 100,000-input campaign, with sanitizer coverage, AddressSanitizer, and UndefinedBehaviorSanitizer; the separate deterministic mutation harness retains its sanitizer gate; independent review and pilot remain pending (see below) |
| M08 expanded coverage | `in_progress` | `src/rh_vcs_hg.elisa` (Mercurial) + `src/rh_vcs_svn.elisa` (Subversion) + `src/rh_vcs_fossil.elisa` (Fossil) + `src/rh_patch.elisa` (non-PR patch series) + `src/rh_release_feed.elisa`/`src/rh_release_feed_report.elisa` (release-only, `rh_cli release-feed`) + `src/rh_pep440.elisa`/`src/rh_pep440_report.elisa` (Python version semantics, `rh_cli pep440`) + `src/rh_marker.elisa` (bounded PEP 508/751 expression evaluator, tested in `test_m08`; pylock context and package/top-level `requires-python` evaluation now run through `rh_cli pylock-evaluate`, with omitted extras defaulting empty and omitted dependency groups using `default-groups`) + `src/rh_gerrit.elisa` (Gerrit review-workflow, `rh_cli vcs --format gerrit`, folds N patch-sets into one change, R025) + `src/rh_spdx.elisa` (SPDX 2.3 inventory, exposed via `rh_cli inventory --format spdx`) + `src/rh_vcs_report.elisa` (`rh_cli vcs --format hg|svn|fossil|patch`, native `--repo` capture) + `src/rh_registry_pypi.elisa`/`src/rh_registry_npm.elisa`/`src/rh_registry_crates.elisa`/`src/rh_registry_rubygems.elisa` (bounded provider metadata normalization) + PyPI `requirements.txt`/PEP 621, NuGet `packages.config` and `packages.lock.json`, and Maven `pom.xml` parsing in `src/rh_package.elisa` (`rh_cli deps`) + bounded Debian `debian/control` distribution metadata (`src/rh_distribution.elisa`, `rh_cli distribution`, `rh-distribution-result/1`, `tests/test_distribution_cli.sh`) + bounded RPM spec preamble metadata (`src/rh_rpm.elisa`, `rh_cli rpm-spec`, `rh-rpm-spec-result/1`, `tests/test_rpm_spec_cli.sh`) + bounded archive metadata (`src/rh_archive.elisa`, `rh_cli archive`, `rh-archive-result/1`, `tests/test_archive_cli.sh`) + bounded Homebrew formula metadata (`src/rh_homebrew.elisa`, `rh_cli homebrew`, `rh-homebrew-result/1`, `tests/test_homebrew_cli.sh`) + bounded Arch `PKGBUILD` assignment metadata (`src/rh_arch.elisa`, `rh_cli arch-pkgbuild`, `rh-arch-pkgbuild-result/1`, `tests/test_arch_pkgbuild_cli.sh`), `connectors/manifests/{mercurial,subversion,fossil,release-feed}.json`, `fixtures/vcs/{svn-log.xml,fossil-timeline.txt}`, `test_m08` (`M08 OK`), `tests/test_m08.sh`, `tests/test_inventory_cli.sh`, `tests/test_vcs_cli.sh`, `tests/test_release_feed_cli.sh`, `tests/test_pep440_cli.sh`; Bitbucket Cloud forge connector (`rh_cli forge normalize --connector bitbucket`, `tests/test_forge_cli.sh`, guarded `--url` capture); more distribution ecosystems and other forge APIs remain (see below) |
| M09 catalog expansion | `in_progress` | admission lint (`tools/metric-lint.sh`) over 226 definitions (224 `implemented`, 2 `prototype`, 0 `planned`); experimental prototypes remain outside the runtime registry (see below) |
...
fenced jobs through migration functions, with bound parameters and
...
finishes the queued collection job. `rh_cli ingest --postgres` now consumes prevalidated normalized pages, commits event and cursor state under the targeted lease, and finalizes complete, partial, failed, or canceled collection runs. Incomplete runs persist coverage/error details, commit only supplied complete/empty pages, and may contain no durable pages; fake-libpq coverage is mandatory and PostgreSQL-backed execution is included in the opt-in live rehearsal. The default `rh_cli ingest --root` path remains filesystem-backed. GitHub repository normalization can authenticate with `RH_GITHUB_TOKEN` on the exact `api.github.com` host; bounded workflow-event acquisition includes repository review batches; approved Gitea/Forgejo connector instances can run three fixed one-item read probes for issues, pulls, and releases through the shared HTTPS/DNS guard; Bitbucket Cloud probes its documented one-item issue and pull-request list routes, while releases are explicitly unsupported because Cloud exposes downloads rather than release objects (`rh_cli connector probe --bitbucket-repo workspace/repo --all`, `tests/test_connector_cli.sh`); collaborator access is checked with an authenticated one-row probe that omits account fields from the result; evidence blobs can move between roots through the guarded transfer package; the fixed GitHub traffic probe and bounded non-additive traffic response normalizer are implemented. `connector probe --review-pull N` tests one fixed review-list route for an explicit pull request and retains separate request/response evidence. `connector probe --collaborators` requires a token, tests one list row, and keeps account details out of the versioned result. GitLab route probes test issues, merge requests, and releases independently through the fixed API host. Probe capabilities retain provider status/HTTP evidence and map to shared coverage states, with ambiguous 403/404 and rate limits reported as unavailable. The PostgreSQL adapter now also exposes
...
`rh_cli staged-normalize` consumes committed GitHub, GitLab, Gitea, Forgejo,
and Bitbucket staged rows through their allowlisted capability tokens. Its
`rh-postgres-staged-normalize-result/1` binds exact input bytes, page/ordinal
...
current-state projection for one exact source/capability/scope tuple. Only
complete or successful-empty acquisition marks prior unseen native IDs absent;
partial, failed, and unsupported acquisition keeps them unconfirmed, while
observed IDs remain present evidence. The result exposes
`refresh_last_success`, which stays false for incomplete or failed acquisition.
...
records (observed bytes remain null), and `peerDependenciesMeta` optional-peer
conditions on both resolved and unresolved graph entries; unsupported or
mismatched revisions rejected). Pnpm lockfile version 9.0 resolves one root
importer through exact package and snapshot keys, preserves normal/dev/optional
scope and peer-suffixed resolution instances, retains integrity and
fingerprinted tarball locators, and leaves workspace links context-unresolved;
other revisions, workspace importers, aliases, malformed entries, and unknown
fields fail closed. Node lock selection prefers npm shrinkwrap, npm package
lock, pnpm, then Yarn Classic v1. Yarn maps exact name/range selectors, retains
dependency scopes and fingerprinted resolved locators, and rejects Berry
revisions, aliases, malformed entries, and unknown fields. Explicit
...
`rh_cli artifact-observe --graph <package-graph> --artifact <id> --file
<local-artifact> --out <file>` verifies raw Cargo archive bytes with SHA-256,
npm tarball bytes with SHA-256/SHA-384/SHA-512 SRI Base64, or Go `go.mod`
content using its `/go.mod` h1 checksum, then emits a sidecar bound to the exact
graph digest and artifact ID after an 8 MiB graph-file check before read and a
64 MiB file cap; changed bytes under the same
...
bounded path does not extract or execute archives; Go module ZIP hashing uses the
bounded `rh_go_zip` reader and system zlib for raw DEFLATE/CRC32, while ZIP64,
split, encrypted, duplicate-name, malformed, and over-expanded archives fail
closed. Integrity strings without SHA-256/SHA-384/SHA-512 and graph rewrites
remain unsupported.
...
it reads a local project's `Cargo.lock`, `package.json` plus
`npm-shrinkwrap.json`, `package-lock.json`, pnpm v9.0 `pnpm-lock.yaml`, or Yarn
Classic v1 `yarn.lock` in the documented precedence, `requirements.txt`, bounded PEP 621 `pyproject.toml`,
...
with no partial graph; advisories in this graph path are only collected from a provided
OSV response file or a guarded `--osv-url` capture, never fabricated. The
separate `rh_cli osv-query --input <rh-osv-query-input/1|2> --out <dir>` path
queries one package with a supplied version string or one full commit hash at
the fixed OSV endpoint and writes a normalized response plus retained
request/raw/status/hash evidence; package responses can feed `deps --osv`,
while commit responses remain separate context evidence because Git-range
matching is not implemented. Default mode retains the v1 result contract and
...
evidence plus the query-to-graph-node index map, and reports per-query advisory
IDs/modification summaries only; it does not produce full records for
`deps --osv`. The opt-in `/2` path follows only per-query cursors for at most
four rounds and records each page's original query indexes; an unfinished
round saves a remaining request and stays partial. Opt-in
`--hydrate-advisories` emits batch result `/3`, fetches at most 64 distinct
advisories, 32 MiB, and 120 seconds total (10 seconds per request) through the
fixed OSV vulnerability lookup, rejects unsafe IDs and mismatched returned IDs, and retains request,
raw response, HTTP status, stderr, and response digest per attempted record.
The emitted `osv-query-hydrated-response.json` has the full-record shape used
by the offline matcher. Any invalid ID, provider failure, byte/record cap, or
unfinished query makes the envelope partial; it does not imply complete
lockfile coverage. OSV documents server-side version matching as fuzzy, and
capture time does not establish feed freshness. Complete lockfile coverage
remains open.
...
`tests/test_deps_cli.sh` checks the supported Cargo, npm, pnpm v9.0, Yarn,
...
the fail-closed branches. A separate bounded PEP 751 audit path,
`rh_cli pylock --input <pylock.toml> --out <file>`, emits
`rh-pylock-audit/1` package-to-package evidence. It preserves package markers
and Python constraints without evaluating them, and preserves top-level
environment, extra, dependency-group, and default-group declarations without
selecting them. It resolves a reference only when name plus optional version
identifies one package entry, and reports missing, ambiguous, or source-specific
references distinctly. Because PEP 751 omits project-root requirements and
labels these links informational, the result is bound to exact input bytes by
SHA-256, not an install graph or OSV input; bounded wheel, sdist, and archive
hash algorithm/value pairs are retained as expected artifact evidence; optional
...
remain unsupported, and no archive is opened or extracted. Pylock context evaluation now emits bounded, digest-bound environment and package-marker coverage with validated extras, dependency groups, and Python constraints. Complete artifact metadata, unsupported dependency selector fields, and complete TOML grammar remain coverage gaps. The TOML string reader decodes standard single-line basic-string escapes, including Unicode escapes as UTF-8, respects escaped quotes while locating comments, and fails closed on invalid Unicode scalars. Bounded triple-quoted scalar strings preserve content and normalize line endings; multiline basic-string line continuations remove the escaped newline and following whitespace; closing quote runs of three to five quotes preserve the TOML-defined content; longer runs fail closed. The reusable `src/rh_marker.elisa` evaluator handles bounded PEP 508/PEP 751 boolean expressions, scalar and set membership, PEP 440 comparisons, and unknown context; `tests/test_m08.sh` exercises precedence, parentheses, groups, extras, and malformed expressions.
`rh_cli pylock-project --project <pyproject.toml> --audit <rh-pylock-audit/1> [--context <rh-pylock-evaluation-context/1>] --out <file>` emits `rh-pylock-project-membership/1`: it reports PEP 621 direct declarations against normalized package-name and byte-exact version-label membership, preserves optional scope and group names (supported bounded bare/quoted keys; unsupported dotted bare and escaped keys fail closed), hashes declarations instead of publishing direct-reference URL text, and binds the project and audit digests plus the selected context digest when supplied. The `==`, `!=`, `<`, `<=`, `>`, `>=`, and `~=` PEP 440 operators and stable version-prefix `==X.*`/`!=X.*` specifiers are evaluated against stable lock versions; prerelease/local versions and unsupported version forms stay unevaluated. Optional `--context` evaluates declaration markers with true/false/unknown results, bound by the exact context digest; marker applicability remains separate from lock membership, and the report is not an installation graph. Its transformation sidecar records source, configuration, and output digests. `tests/test_pylock_project_cli.sh` checks exact/ranged/version-prefix membership, named optional groups, true/false/unknown markers, context digest binding, missing candidates, canonicalized names, secret redaction, and malformed inputs.
`tests/test_pylock_cli.sh`, its audit and
...
source links typed as repository/homepage/project URL/source code while
retaining `repository` as a compatibility alias, each labelled an assertion,
and exact
...
covers canonical, npm, NuGet, and PyPI project shapes, typed links, the
4096-byte notice bound, source lineage, and fail-closed negatives; the public result is checked
...
malformed records fail closed. `rh_cli artifact-observe` verifies a supplied
`go.mod` against its one-file `h1` summary and emits
`rh-go-mod-observation-result/1`; Go module ZIP bytes now follow sorted-name,
uncompressed-content `h1` semantics and emit `rh-go-zip-observation-result/1`.
Registry metadata now also has a bounded `https`/`file`
...
--out <dir>` parses the pinned `evidence/git-log.bin` into project-local author-identity events grouped by the repository `.mailmap` (`src/rh_continuity_scan.elisa`),
...
Actor ids after mailmap normalization are assigned by first appearance; only
explicit repository-local mailmap assertions group aliases, with no automatic
display-name/domain or cross-source merge. Reversible identity links are a real path
...
explicitly subscribed, an eligible `new` decision is marked `dispatchable`
(external delivery is not performed by this command), and a refusal records
nothing (no phantom cooldown),
a changed artifact digest is a new key, and ack/resolve silence a key.
The input carries a bounded unique-ID destination allowlist; events name only
a configured destination ID, and cannot supply target or authorization fields.
Unknown IDs are refused before state changes. Cooldown, acknowledgment, and
resolution keys include the destination ID, so one subscriber does not
suppress another. Input/result/state are versioned `/3`, `/4`, and `/2`;
...
`rh_cli notify --state <file>` loads a prior `rh-notify-state/2` and writes
...
`rule_id`/`subject_id`/`digest`/`context_digest`/`expires_at`/`state` plus
`approved_by`/`reason` provenance for approvals and writes the
...
context re-binding, approval provenance replay, and malformed-state exit 4.
The recorded approver is caller-supplied and is not cryptographically
authenticated. Public contract `policy-state` added
...
the deterministic sanitizer harness remains separately enabled. A fresh
100,000-run campaign from those seeds completed without a crash with Homebrew
clang 23.1.1; coverage counters rose from 1,197 to 1,876. Earlier iterations
found two malformed NuGet XML span cases and patch-series decimal overflow;
the parsers now fail safely and the minimized inputs are regression fixtures.
Further fuzzing and independent review remain open. The beta gate is **not** passed; `M07` stays
`in_progress`.
...
metric cannot be a bare name. Every definition also declares `raw`,
`derived`, or `modeled`; modeled definitions must remain experimental. The
canonical commit Contributor Absence Factor definition and its compatibility
alias carry a source-identified CHAOSS crosswalk with local population,
inclusive threshold boundary, empty-population state, differences, and
interpretation limit. `tests/test_metric_admission.sh` exercises the metadata
and rejects removal of a class or changes to the CHAOSS boundary. The last
`planned` definition,
...
available) while published definitions are present. Catalog state: 226
definitions = 224 `implemented`, 2 `prototype`, 0 `planned`; the 360 target
...
87 public machine-readable contracts, including the research lineage,
...
"implemented"`** (computed on the real scan path). It lists 224 reachable
...
sample counts in
`rh-forecast-result/2`; the adapter validates supplied evaluation evidence
without independently verifying split membership or cohort identity, is labelled experimental,
...
correction replay or worker-scheduler load shedding. Worker unit and CLI tests now
cover due-full priority and deterministic source rotation under bounded capacity;
they do not establish fairness under deployed worker contention. `RH_PROFILE_REPO` now opts
...
**Research-paper follow-up status (2026-09-21):** RP-02 now has a real
...
metric classes (`tests/test_lineage_cli.sh`). The staged forge adapter now
...
## Metric catalog status (224 implemented definitions; 225 Architecture keys lack an implemented definition)
...
| succession.activity_primary_actor_changes | 1.0.0 | `implemented` (F017; adjacent complete months with distinct unique activity maxima) |
...
| adoption.observed_upgrade_count | 1.0.0 | `implemented` (M05-07; complete first/latest version evidence with a strictly newer latest ordinal) |
| adoption.version_comparison_count | 1.0.0 | `implemented` (M05-07; denominator for observed-upgrade comparisons) |
| adoption.supported_line_adoption_count | 1.0.0 | `implemented` (M05-07; latest observed version matches the supplied supported major) |
| adoption.supported_line_assessed_count | 1.0.0 | `implemented` (M05-07; denominator for supported-line adoption; absent declarations stay unknown) |
| adoption.duration_confirmed_count | 1.0.0 | `implemented` (M05-07; confirmed introduction and removal provide an observed duration) |
| adoption.duration_right_censored_count | 1.0.0 | `implemented` (M05-07; confirmed introduction without confirmed removal by last-seen/cutoff) |
| adoption.duration_unknown_count | 1.0.0 | `implemented` (M05-07; no confirmed introduction means duration is unknown) |
| adoption.confirmed_duration_distribution | 1.0.0 | `implemented` (M05-07; five fixed elapsed-time buckets for confirmed introduction-to-removal durations) |
| adoption.right_censored_duration_distribution | 1.0.0 | `implemented` (M05-07; same fixed buckets, reported separately through last-seen/cutoff) |
| adoption.observed_upgrade_lag_distribution | 1.0.0 | `implemented` (M05-07; elapsed seconds from upstream release to the first comparable downstream snapshot meeting the target version) |
| adoption.right_censored_upgrade_lag_distribution | 1.0.0 | `implemented` (M05-07; lag distribution only where complete comparable follow-up through the stated boundary found no qualifying snapshot or coverage gap) |
| adoption.snapshot_coverage_gap_count | 1.0.0 | `implemented` (M05-07; uncovered intervals in supplied coverage or reconstructed snapshot-run history) |
| adoption.snapshot_coverage_uncovered_seconds | 1.0.0 | `implemented` (M05-07; elapsed seconds not covered by supplied intervals or reconstructed snapshot runs) |
...
| downstream_condition.supported_version_adoption_share | 1.0.0 | `implemented` (F021, F023, F024; exact supported-major adoption numerator over relations with explicit latest-version and supported-major evidence) |
...
| 10 | Extend to the initial 30–40 metric subset | `implemented` (224 metric definitions are implemented; registry/status agreement and admission lint pass in `tests/test_m00.sh`) |
| 11 | PostgreSQL ingestion, cursor transaction, and lease tests | `partial` (the PostgreSQL 16 migration rehearsal covers idempotent source/run and collection-job enqueue/replay, conflicting metadata rejection, enqueue/claim/atomic finish, evidence registration, atomic subject/actor/event/page/cursor commit, run-mismatched and stale/expired lease rejection, duplicate absorption, malformed-page rollback, complete-only cursor advancement, and crash/restart recovery of an interrupted page transaction; `rh_cli postgres` exposes parameterized source/run, enqueue, evidence, page/event-page, and job methods, and `rh_cli ingest --postgres` executes preflighted normalized pages through the full run/job/page lifecycle with fake-libpq and opt-in live coverage, while provider acquisition/authentication remain open; evidence blobs use the guarded, corruption-checked transfer package) |
