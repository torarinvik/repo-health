...
| M07 beta gate | `in_progress` | `src/rh_ops.elisa` + `src/rh_ops_report.elisa` (`rh_cli ops monitor|quota`) + `src/rh_privacy.elisa` + `src/rh_privacy_report.elisa` (`rh_cli privacy`) + `src/rh_sha256.elisa` (SHA-256/HMAC, M07-04 sign/verify) + transport guard (`src/rh_git.elisa`) + `ops/source-review-register.json`, `test_m07` (`M07 OK`), `test_m07_sha`, `tests/test_m07.sh`/`test_m07_sanitizer.sh`/`test_m07_fuzz.sh`/`test_m07_sha.sh`/`test_ops_cli.sh`/`test_privacy_cli.sh`; the Elisa parser target now runs a bounded 1,000-input LLVM libFuzzer campaign with sanitizer coverage, AddressSanitizer, and UndefinedBehaviorSanitizer; the separate deterministic mutation harness retains its AddressSanitizer/UndefinedBehaviorSanitizer gate; sustained fuzzing, independent review, and pilot remain pending (see below) |
...
campaign, crash-corpus triage, and independent review remain open. The beta
gate is **not** passed; `M07` stays
`in_progress`.
