...
`rh_cli artifact-observe --graph <cargo-graph> --artifact <id> --file
<local-archive> --out <file>` verifies raw Cargo archive bytes using SHA-256 and
emits a sidecar bound to the exact graph digest and artifact ID; changed bytes
under the same package/version are reported as `changed`. This bounded path
does not extract or execute archives and does not yet verify npm/Go digest
formats or rewrite graph records.
...
80 public machine-readable contracts, including the research lineage,
