...
| M09 catalog expansion | `in_progress` | admission lint (`tools/metric-lint.sh`) over 224 definitions (222 `implemented`, 2 `prototype`, 0 `planned`); experimental prototypes remain outside the runtime registry (see below) |
...
available) while published definitions are present. Catalog state: 224
definitions = 222 `implemented`, 2 `prototype`, 0 `planned`; the 360 target
...
"implemented"`** (computed on the real scan path). It lists 222 reachable
...
## Metric catalog status (222 implemented definitions; 225 Architecture keys lack an implemented definition)
...
| adoption.right_censored_upgrade_lag_distribution | 1.0.0 | `implemented` (M05-07; lag distribution only where complete comparable follow-up through the stated boundary found no qualifying snapshot) |
...
| 10 | Extend to the initial 30–40 metric subset | `implemented` (222 metric definitions are implemented; registry/status agreement and admission lint pass in `tests/test_m00.sh`) |
