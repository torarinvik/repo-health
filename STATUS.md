...
<file>` process an `rh-notify-input/2` event stream (notify/ack/resolve)
into `rh-notify-result/2`, where an upstream maintainer is refused unless
...
Configured subscriber, maintainer-self, and operator destinations require an
explicit `authorized:true`; a missing flag fails closed before state changes.
The incompatible authorization default was versioned from input/result `/1`
to `/2`; legacy input is rejected. `tests/test_notify_cli.sh` verifies the
authorization regression, notification state transitions, and fail-closed
inputs.
