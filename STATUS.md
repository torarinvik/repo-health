...
finishes the queued collection job. `rh_cli ingest --postgres` now consumes prevalidated normalized pages, commits event and cursor state under the targeted lease, and finalizes complete or partial collection runs. Partial runs persist coverage/error details, commit only supplied complete/empty pages, and may contain no durable pages; fake-libpq coverage is mandatory and PostgreSQL-backed execution is included in the opt-in live rehearsal. The default `rh_cli ingest --root` path remains filesystem-backed, while provider acquisition/authentication, terminal failed/canceled collection jobs, and shared-blob distribution remain open. The PostgreSQL adapter now also exposes
