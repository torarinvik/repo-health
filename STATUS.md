...
`rh_cli pylock-project --project <pyproject.toml> --audit <rh-pylock-audit/1> --out <file>` emits `rh-pylock-project-membership/1`: it reports PEP 621 direct declarations against normalized package-name and byte-exact version-label membership, keeps optional scope, hashes declarations instead of publishing direct-reference URL text, and binds both input digests. Ranges, markers, and resolver satisfaction remain unevaluated; the report explicitly is not an installation graph. Its transformation sidecar records source, configuration, and output digests. `tests/test_pylock_project_cli.sh` checks exact, ranged, optional, missing, canonicalized-name, secret-redaction, and malformed-audit cases.
