...
remain unsupported, and no archive is opened or extracted. Pylock context
evaluation and environment/group selection, complete artifact metadata, unsupported dependency
selector fields, and complete TOML grammar remain coverage gaps. The reusable
`src/rh_marker.elisa` evaluator now handles bounded PEP 508/PEP 751 boolean
expressions, scalar and set membership, PEP 440 comparisons, and unknown
context; `tests/test_m08.sh` exercises precedence, parentheses, groups, extras,
and malformed expressions. It is not yet wired to pylock context selection.
