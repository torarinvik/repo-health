...
| 11 | PostgreSQL ingestion, cursor transaction, and lease tests | `partial` (the PostgreSQL 16 migration and live fencing/cursor function rehearsal passed on 2026-09-20; `src/rh_postgres.elisa` now exposes a parameterized `libpq` page-commit method with ABI fixture coverage, while event/evidence/job application methods, ingestion wiring, and database crash/restart boundary tests remain open) |
