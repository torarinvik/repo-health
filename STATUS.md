...
finishes the queued collection job. `rh_cli ingest --postgres` now consumes prevalidated normalized pages, commits event and cursor state under the targeted lease, and finalizes complete, partial, failed, or canceled collection runs. Incomplete runs persist coverage/error details, commit only supplied complete/empty pages, and may contain no durable pages; fake-libpq coverage is mandatory and PostgreSQL-backed execution is included in the opt-in live rehearsal. The default `rh_cli ingest --root` path remains filesystem-backed. GitHub repository normalization can authenticate with `RH_GITHUB_TOKEN` on the exact `api.github.com` host; bounded workflow-event acquisition includes repository review batches; broader live capability probes, collaborator permission acquisition, and shared-blob distribution remain open; the fixed GitHub traffic probe is implemented. The PostgreSQL adapter now also exposes
