...
`rh_cli pylock-project --project <pyproject.toml> --audit <rh-pylock-audit/1> --out <file>` emits `rh-pylock-project-membership/1`: it reports PEP 621 direct declarations against normalized package-name and byte-exact version-label membership, preserves optional scope and group names (supported bounded bare/quoted keys; unsupported dotted bare and escaped keys fail closed), hashes declarations instead of publishing direct-reference URL text, and binds both input digests. The `==`, `!=`, `<`, `<=`, `>`, and `>=` PEP 440 specifier operators are evaluated against stable lock versions; prerelease/local versions, compatible-release and wildcard specifiers, markers, and unsupported forms remain unevaluated, and the report explicitly is not an installation graph. Its transformation sidecar records source, configuration, and output digests. `tests/test_pylock_project_cli.sh` checks exact, ranged, named optional-group, missing, canonicalized-name, secret-redaction, and malformed-audit cases.
