...
| M06 API/policy/corrections | `in_progress` | `src/rh_policy.elisa` + `src/rh_policy_parse.elisa` + `src/rh_correction.elisa` + `src/rh_correction_report.elisa` + `src/rh_notify.elisa` + `src/rh_notify_report.elisa` + `src/rh_query.elisa` + `src/rh_query_report.elisa` + `src/rh_render.elisa` + `src/rh_findings.elisa` + `src/rh_findings_transformation.elisa` + `src/rh_lineage.elisa` + `src/rh_drilldown.elisa`, `test_m06`/`test_m06b`, `rh_cli policy`/`correct`/`notify`/`query`/`render`/`findings`/`lineage`/`drilldown` publish `rh-policy-result/1`/`rh-policy-state/1`/`rh-corrections-result/1`/`rh-corrections-state/1`/`rh-notify-result/4`+`rh-notify-state/2`/`rh-query-result/1`/`repo-health-report-html/1`/`rh-findings-result/1`/`rh-lineage-result/1`/`rh-evidence-drilldown/1`, `rh_cli serve /_report.html` is server-rendered; `tests/test_m06.sh`, `tests/test_policy_cli.sh`, `tests/test_correction_cli.sh`, `tests/test_notify_cli.sh`, `tests/test_query_cli.sh`, `tests/test_render_cli.sh`, `tests/test_findings_cli.sh` (typed outcomes and transformation-sidecar digests), `tests/test_lineage_cli.sh`, `tests/test_drilldown_cli.sh`; dynamic UI tables, fixed report-backed API resource routes, capped upstream/downstream query traversal with truncation metadata, verified `/api/store/<16-hex-digest>` evidence lookup, structured findings, lineage-preserving evidence drill-down, and durable state-store wiring are implemented; correction inputs now require nonempty per-dispute `evidence_ref` values and reviewer/time metadata on accepted or rejected decisions, preserving all fields in results; optional `--evidence-store` recomputes and verifies referenced content-addressed blobs before applying corrections; newly accepted corrections emit idempotent `rh-correction-notices/1` artifacts with revision and review/evidence lineage; registration/access policy and notice delivery remain open |
