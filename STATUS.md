...
separate `rh_cli osv-query --input <rh-osv-query-input/1> --out <dir>` path
queries one package with the exact supplied version string at the fixed OSV endpoint and writes a
matcher-ready response plus retained request/raw/status/hash evidence;
...
79 public machine-readable contracts, including the research lineage,
