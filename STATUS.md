...
finishes the queued collection job. `rh_cli ingest --postgres` now consumes prevalidated normalized pages, commits event and cursor state under the targeted lease, and finalizes complete, partial, failed, or canceled collection runs. Incomplete runs persist coverage/error details, commit only supplied complete/empty pages, and may contain no durable pages; fake-libpq coverage is mandatory and PostgreSQL-backed execution is included in the opt-in live rehearsal. The default `rh_cli ingest --root` path remains filesystem-backed. GitHub repository normalization can authenticate with `RH_GITHUB_TOKEN` on the exact `api.github.com` host; bounded workflow-event acquisition includes repository review batches; provider-wide probes remain open; collaborator access is checked with an authenticated one-row probe that omits account fields from the result; evidence blobs can move between roots through the guarded transfer package; the fixed GitHub traffic probe and bounded non-additive traffic response normalizer are implemented. `connector probe --review-pull N` tests one fixed review-list route for an explicit pull request and retains separate request/response evidence. `connector probe --collaborators` requires a token, tests one list row, and keeps account details out of the versioned result. Probe capabilities retain provider status/HTTP evidence and map to shared coverage states, with ambiguous 403/404 and rate limits reported as unavailable. The PostgreSQL adapter now also exposes
