...
| M09 catalog expansion | `in_progress` | admission lint (`tools/metric-lint.sh`) over 220 definitions (218 `implemented`, 2 `prototype`, 0 `planned`); experimental prototypes remain outside the runtime registry (see below) |
...
available) while published definitions are present. Catalog state: 220
definitions = 218 `implemented`, 2 `prototype`, 0 `planned`; the 360 target
...
"implemented"`** (computed on the real scan path). It lists 218 reachable
...
## Metric catalog status (218 implemented definitions; 225 Architecture keys lack an implemented definition)
...
| 10 | Extend to the initial 30–40 metric subset | `implemented` (218 metric definitions are implemented; registry/status agreement and admission lint pass in `tests/test_m00.sh`) |
