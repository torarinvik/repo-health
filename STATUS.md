...
| M03 packages/advisories | `in_progress` | `src/rh_package.elisa` (Cargo + npm, pnpm v9.0, Yarn Classic v1 + PyPI requirements/PEP 621 + Go modules + RubyGems + Composer + NuGet `packages.config` and `packages.lock.json` + Maven `pom.xml`) + `src/rh_artifact_observation.elisa` + `src/rh_go_zip.elisa` + `src/rh_inventory.elisa` + `src/rh_inventory_report.elisa` + `src/rh_registry_meta.elisa` + `src/rh_registry_pypi.elisa` + `src/rh_registry_npm.elisa` + `src/rh_registry_crates.elisa` + `src/rh_registry_rubygems.elisa` + `src/rh_registry_nuget.elisa` + `src/rh_registry_meta_report.elisa` + `src/rh_depsdev.elisa` + `src/rh_ecosystem_lookup.elisa` + `src/rh_resolution.elisa` + `src/rh_parser_diff.elisa` + `src/rh_adapter_manifest.elisa` + `src/rh_osv_query.elisa` + `src/rh_pylock.elisa` (PEP 751 rootless audit projection, `rh_cli pylock`, `rh-pylock-audit/1`) + `test_package` (`PACKAGE OK`) + `test_osv_query`, `rh_cli deps` publishes `dependencies.unsupported_range_count`, `rh_cli artifact-observe` publishes graph-bound Cargo SHA-256 archive, npm SHA-256/SHA-384/SHA-512 SRI tarball, Go `.mod` and module ZIP h1 evidence (`rh-artifact-observation-result/1`, `rh-go-mod-observation-result/1`, and `rh-go-zip-observation-result/1`), `rh_cli inventory` publishes `rh-inventory/1`, `rh_cli registry-meta` publishes `rh-registry-meta-result/1` for canonical, PyPI, npm, crates.io, RubyGems, and bounded NuGet registration metadata, `rh_cli depsdev` publishes `rh-depsdev-enrichment/1`, `rh_cli ecosystem lookup` publishes `rh-ecosystem-lookup-result/1` from bounded captures or one explicitly requested fixed-route live lookup, with exact-coordinate candidate accounting and source/status plus CC BY-SA 4.0 attribution, licence, and modification provenance, `rh_cli resolution` publishes `rh-resolution-instance/1`, `rh_cli parser-diff` publishes `rh-parser-diff/1`, and `rh_cli adapter-manifest` publishes `rh-adapter-manifest-result/1`; `tests/test_m03.sh`, `tests/test_deps_cli.sh` (Cargo, npm, pnpm v9.0, and Yarn lockfile coverage), `tests/test_artifact_observation_cli.sh`, `tests/test_inventory_cli.sh`, `tests/test_registry_meta_cli.sh`, `tests/test_depsdev_cli.sh`, `tests/test_ecosystem_lookup_cli.sh`, `tests/test_resolution_cli.sh`, `tests/test_parser_diff_cli.sh`, `tests/test_adapter_manifest_cli.sh`, `tests/test_osv_query_cli.sh`, `tests/test_pylock_cli.sh`; OSV supports bounded package/version and full commit queries plus graph batches of up to 64 nodes; graph result `/2` follows only pending per-query cursors for at most four rounds and retains indexed request/response evidence; opt-in `--hydrate-advisories` result `/3` fetches up to 64 distinct full records and 32 MiB of bodies with a 120-second total and 10-second per-request cap, validates returned IDs, retains per-record evidence, and reports any gap as partial. Complete lockfile coverage and further provider adapters remain open |
