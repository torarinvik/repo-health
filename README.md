...
version constraints without evaluating them, preserves top-level environment,
extra, and dependency-group declarations without selecting them, reports
...
`context`. Package `attestation-identities` retain the required `kind`, publisher string
fields, and inline string-valued `claims` members as recorded; the audit reports
identity data and does not verify attestations. It counts
