...
for p in sorted(glob.glob(os.path.join(root, "schemas", "*.schema.json"))):
    ...
    assert s["targets"], p
    names.add(s["name"])
for want in ("connector-manifest", "canonical-repo", "dep-graph", "artifact-observation-result", "go-mod-observation-result", "go-zip-observation-result", "projection-snapshot", "cyclonedx", "spdx", "registry-meta", "distribution", "archive", "role-publication", "homebrew", "osv-query-input", "osv-commit-query-input", "osv-query-batch-result", "osv-query-batch-pages-result", "osv-query-batch-hydrated-result"):
