...
rm -rf "$T"; mkdir -p "$T"
cat > "$T/in.json" <<'JSON'
{"schema":"rh-release-feed-input/1","first_seen":1700000123,"releases":[{"tag":"v1.0.0","published_at":1700000000,"prerelease":false,"withdrawn":true,"assets":[{"name":"a.tgz","digest":"sha256:abc"},{"name":"b.tgz"}]},{"tag":"v1.1.0","prerelease":true}]}
JSON
...
assert r[0] == {"tag": "v1.0.0", "published_at": 1700000000, "first_seen": 1700000123,
                "prerelease": False, "withdrawn": True, "asset_count": 2, "digest_known_count": 1}, r[0]
# missing published time stays unknown (null), never replaced by first-seen
assert r[1]["published_at"] is None and r[1]["prerelease"] is True and r[1]["withdrawn"] is None and r[1]["first_seen"] == 1700000123, r[1]
