...
{"schema":"rh-identity-publication-input/1","project_id":"example-parser","publication_scope":"public","actor_count":4,"links":[{"a":0,"b":1,"state":"accepted","revision_added":1},{"a":1,"b":2,"state":"accepted","revision_added":2},{"a":3,"b":0,"state":"rejected","revision_added":3}],"actor_kinds":["human","human","unresolved","bot_known"],"correction_requests":[{"id":"c1","state":"open"},{"id":"c2","state":"accepted"},{"id":"c3","state":"rejected"},{"id":"c4","state":"withdrawn"}],"restricted_actor_metadata":[{"source_instance":"forge.example","native_object_id":"account-1","display_name":"Alice","aliases":["alice@example"]}]}
JSON
...
assert d["actor_count"] == 4 and d["identity_revision"] == 2 and d["cluster_count"] == 2, d
assert d["cluster_size_histogram"] == [{"size": 3, "clusters": 1}, {"size": 1, "clusters": 1}], d
assert d["actor_kinds"] == {"human": 2, "bot_known": 1, "unresolved": 1}, d
...
import json, sys
d = json.load(open(sys.argv[1]))
assert d["actor_kinds"] == {"human": 0, "bot_known": 0, "unresolved": 2}, d
