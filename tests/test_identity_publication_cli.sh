...
{"schema":"rh-identity-publication-input/1","project_id":"example-parser","publication_scope":"public","actor_count":5,"links":[{"a":0,"b":1,"state":"accepted","revision_added":1},{"a":1,"b":2,"state":"accepted","revision_added":2},{"a":3,"b":0,"state":"rejected","revision_added":3}],"actor_kinds":["human","human","unresolved","bot_known","service_known"],"actor_kind_sources":["provider","provider","unknown","provider","project"],"actor_kind_observed_at":[1700000000,null,1700000200,1700000300,1700000400],"correction_requests":[{"id":"c1","state":"open"},{"id":"c2","state":"accepted"},{"id":"c3","state":"rejected"},{"id":"c4","state":"withdrawn"}],"restricted_actor_metadata":[{"source_instance":"forge.example","native_object_id":"account-1","display_name":"Alice","aliases":["alice@example"]}]}
JSON
...
assert "1700000000" not in text and "1700000400" not in text, text
...
set -e
for rc in "$rc_schema" "$rc_scope" "$rc_link" "$rc_duplicate" "$rc_kind_time" "$rc_kind_time_count"; do
    [[ "$rc" -eq 4 ]] || fail "malformed publication must exit 4 (got $rc)"
done
for path in "$T/bad-schema.out" "$T/bad-scope.out" "$T/bad-link.out" "$T/bad-duplicate.out" "$T/bad-kind-time.out" "$T/bad-kind-time-count.out"; do
