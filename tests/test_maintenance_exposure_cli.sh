...
assert pairs[1]["subject_evidence_ref"] == "aaaaaaaaaaaaaaaa" and pairs[1]["dependent_evidence_ref"] == "bbbbbbbbbbbbbbbb", pairs[1]
...
with open(sys.argv[2], "w") as f:
    ...
    f.write("\n")
PY
"$ROOT/build/rh_cli" maintenance-exposure --input "$T/incomplete.json" --out "$T/incomplete.out" >/dev/null || fail "incomplete evidence should preserve unknown state"
...
PY
for bad in unaccepted bad-evidence bad-role bad-ledger duplicate; do
  set +e
  "$ROOT/build/rh_cli" maintenance-exposure --input "$T/$bad.json" --out "$T/$bad.out" >/dev/null 2>&1
  rc=$?
  set -e
