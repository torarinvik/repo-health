...
rm -rf "$T"; mkdir -p "$T"
cat > "$T/in.json" <<'JSON'
{"schema":"rh-pep440-input/1","versions":["1.0.post1","1.0","1.0rc1","1.0b1","1.0a1","1.0a1.dev1","2.0","1!0.1","1.0.1","1.0+local","1.0+abc","1.0+abd","1.0+ABC","1.0+a","1.0+a.1","1.0+2","1.0+10","1.0+a_b","1.0+a.b","v1.0","1..2"]}
JSON
...
assert d["schema"] == "rh-pep440-result/2", d
...
assert tr["configuration_sha256"] == hashlib.sha256(b"repo-health/python-pep440/2").hexdigest(), tr
assert {f["state"] for f in tr["fields"]} == {"preserved", "transformed", "unsupported", "discarded"}, tr
assert d["sorted"] == ["1.0a1.dev1", "1.0a1", "1.0b1", "1.0rc1", "1.0", "1.0+a", "1.0+a_b", "1.0+a.b",
                       "1.0+a.1", "1.0+abc", "1.0+ABC", "1.0+abd", "1.0+local", "1.0+2", "1.0+10",
...
assert by["1.0+local"]["local_segment_count"] == 1, by["1.0+local"]
assert by["1.0+a_b"]["local_segment_count"] == 2, by["1.0+a_b"]
