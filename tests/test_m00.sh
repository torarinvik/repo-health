...
[[ "$keys_count" == "100" ]] || { echo "[m00] FAIL: expected 100 definitions, got $keys_count"; exit 1; }
...
assert impl == 98, ("implemented definitions", impl)
print("[m00] registry mirror OK: published are present, planned are absent; 98 implemented")
EOF
...
grep -q "262 of the 360" "$ROOT/STATUS.md" || { echo "[m00] FAIL: STATUS.md must scope the 360-metric catalog"; exit 1; }
