...
[[ "$keys_count" == "83" ]] || { echo "[m00] FAIL: expected 83 definitions, got $keys_count"; exit 1; }
...
assert impl == 81, ("implemented definitions", impl)
print("[m00] registry mirror OK: published are present, planned are absent; 81 implemented")
EOF
...
grep -q "279 of the 360" "$ROOT/STATUS.md" || { echo "[m00] FAIL: STATUS.md must scope the 360-metric catalog"; exit 1; }
