...
[[ "$keys_count" == "46" ]] || { echo "[m00] FAIL: expected 46 definitions, got $keys_count"; exit 1; }
...
assert impl == 44, ("implemented definitions", impl)
print("[m00] registry mirror OK: published are present, planned are absent; 44 implemented")
EOF
...
grep -q "316 of the 360" "$ROOT/STATUS.md" || { echo "[m00] FAIL: STATUS.md must scope the 360-metric catalog"; exit 1; }
