...
[[ "$keys_count" == "92" ]] || { echo "[m00] FAIL: expected 92 definitions, got $keys_count"; exit 1; }
...
assert impl == 90, ("implemented definitions", impl)
print("[m00] registry mirror OK: published are present, planned are absent; 90 implemented")
EOF
...
grep -q "270 of the 360" "$ROOT/STATUS.md" || { echo "[m00] FAIL: STATUS.md must scope the 360-metric catalog"; exit 1; }
