...
[[ "$keys_count" == "44" ]] || { echo "[m00] FAIL: expected 44 definitions, got $keys_count"; exit 1; }
...
assert impl == 42, ("implemented definitions", impl)
print("[m00] registry mirror OK: published are present, planned are absent; 42 implemented")
EOF
...
grep -q "318 of the 360" "$ROOT/STATUS.md" || { echo "[m00] FAIL: STATUS.md must scope the 360-metric catalog"; exit 1; }
