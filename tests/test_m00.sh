...
[[ "$keys_count" == "67" ]] || { echo "[m00] FAIL: expected 67 definitions, got $keys_count"; exit 1; }
...
assert impl == 65, ("implemented definitions", impl)
print("[m00] registry mirror OK: published are present, planned are absent; 65 implemented")
EOF
...
grep -q "295 of the 360" "$ROOT/STATUS.md" || { echo "[m00] FAIL: STATUS.md must scope the 360-metric catalog"; exit 1; }
